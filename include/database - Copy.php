...
class MySQLDB
{
   ...
   {
   $res=$this->query('select q_id from questions where top_id='.$topid );
   ...
   {
   $res=$this->query("select q_ans from questions where q_id=$qid");
  ...
  { 
  $res=$this->query('select count(*) from questions where top_id='.$topid );
  $row=mysql_fetch_array($res);
  if($row[0]>=$size)
  return true; 
  else
  ...
  {
  $res=$this->query('select count(*) from exam_res where username="'.$stdid.'"');
  $row=mysql_fetch_array($res);
  if($row[0]>0)
  return false;
  else 
  ...
  {
  $res=$this->query('select count(*) from exam_res where exam_id='.$examid);
  $row=mysql_fetch_array($res);
  if($row[0]>0)
  return true;
  else 
  ...
  {
  $res=$this->query('select * from exam_res where username="'.$stdid.'" ORDER BY `exam_res`.`timestamp` DESC');
  while($row=mysql_fetch_array($res))
  ...
  {
  $res=$this->query("select top_title  from topics where top_id=".$topid);
  $row=mysql_fetch_array($res);
  return $row[0];
  }
  function getresults($id)
  {
  $res=$this->query('select * from exam_res where exam_id='.$id);
  $row=mysql_fetch_array($res);
  ...
  {
  $res=$this->query("select * from exam_res where timestamp=(select max(timestamp) from exam_res where username='".$username."')");
  $row= mysql_fetch_array($res);
  return $row[0];
  }
  ...
  $res=$this->query("select top_id,top_title from topics ".$where);
  while($row=mysql_fetch_array($res))
  ...
  $res=$this->query("select sub_id,sub_title from subjects ".$where);
  while($row=mysql_fetch_array($res))
  ...
  {
  global $database;
  $sql="select bran_name from branch where bran_id=$branid";
  $row=mysql_fetch_array($database->query($sql));
  return $row[0];
  }
  function getsubname($subid)
  {
  $sql="select sub_title from subjects where sub_id=$subid";
