...
class Session
{
   ...
   var $branchid;      //branch of loggin user
   ...
   function checkLogin(){
      ...
      if(isset($_SESSION['username']) && isset($_SESSION['userid']) &&
         ...
         $this->branchid = $this->userinfo['branch_id'];
         return true;
      }
      /* User not logged in */
      else{
    ...
    */
   function register($subuser, $subpass, $subemail,$subcaptcha,$subbranch){
      ...
      /* captcha validation*/
     $field="captcha";
     if(!$subcaptcha || strlen($subcaptcha = trim($subcaptcha)) == 0)
     $form->setError($field,"* enter the code displayed");
     include("securimage.php");
     $img = new Securimage();
     $valid = $img->check($subcaptcha);
     if(!$valid){$form->setError($field,"* enter the code correctly"); }
      ...
      /* No errors, add the new account to the */
         ...
         if($database->addNewUser($subuser, md5($subpass), $subemail,$subbranch)){
            if(EMAIL_WELCOME){
    ...
    * automatically.
    */
   function editAccount($subcurpass, $subnewpass, $subemail,$subbranid){
      global $database, $form;  //The database and form object
      ...
      if($subbranid)
      {
        $database->updateUserField($this->username,"branch_id",$subbranid);
      }
  ...
  function startExam($topic_id,$marks) //starts exam taking topic id as input
  {
   ...
   $_SESSION['corcount']=0;
   $db=new MYSQLDB;
   if($db->isdreqs($topic_id,$marks))
   {
   $_SESSION['currentqarray']=$db->getranqarray($topic_id,$marks);
   return true;
   }
   else
   {return false;}
  ...
  { 
   $db=new MYSQLDB;
  $res=$db->query('select q_text from questions where q_id='.$_SESSION['currentqarray'][$_SESSION['currentqkey']]);
  ...
  {
  $db=new MYSQLDB;
   $res=$db->query('select q_op1,q_op2,q_op3,q_op4 from questions where q_id='.$_SESSION['currentqarray'][$_SESSION['currentqkey']]);
   $row=mysql_fetch_array($res);
   for($i=0;$i<4;$i++)
