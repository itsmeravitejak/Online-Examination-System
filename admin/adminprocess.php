...
class AdminProcess
{
   ...
   function AdminProcess(){
      ...
      else if(isset($_POST['chgpass']))
      {
        global $database;
        if(!strlen($_POST['subnewpass']) < 4){
         $database->updateUserField($_POST['subuser'],"password",md5($_POST['subnewpass']));
         echo "true";
        }
      ...
      else if(isset($_POST['delq']))
      {
      $this->procDelq();
      }
      
      else if(isset($_POST['delsubj']))
      {
      $this->procDelsubj();
      }
      else if(isset($_POST['addtop']))
      {
      $this->procAddtop();
      }
   ...
   {
   global $database;
   $q="DELETE FROM questions WHERE q_id =".$_POST['qid'];
   $database->query($q);
   }
   function procDelsubj()
   {
   global $database;
   $q="DELETE FROM subjects WHERE sub_id =".$_POST['subid'];
   $database->query($q);
   }
   function procDeltop()
   {
   global $database;
   $q="DELETE FROM topics WHERE top_id =".$_POST['topid'];
   $database->query($q);
   }
   function procDelbran()
   {
   global $database;
   $q="DELETE FROM branch WHERE bran_id =".$_POST['branid'];
   $database->query($q);
   }
   function procAddbran()
   {
    global $database;
    $q="INSERT INTO branch Values(NULL,'".$_POST['branname']."')";
    $database->query($q);
   }
   function procAddsub()
   {
    global $database;
    $q="INSERT INTO subjects Values(NULL,'".$_POST['subname']."',".$_POST['branid'].")";
    $database->query($q);
   }
   function procAddtop()
   {
    global $database;
    $q="INSERT INTO topics Values(NULL,'".$_POST['topname']."',".$_POST['subid'].")";
    $database->query($q);
   }
